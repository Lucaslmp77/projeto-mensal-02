...
public class AlunoController {
    ...
    @GetMapping("/{idAluno}")
    public ResponseEntity<Aluno> findById(@PathVariable("idAluno") Long idAluno) {
        return ResponseEntity.ok().body(this.alunoService.findById(idAluno));
    }

    @PutMapping("/{idAluno}")
    public ResponseEntity<?> update(@PathVariable Long idAluno, @RequestBody Aluno aluno) {
        try {
    ...
    public ResponseEntity<?> findAlunoByNome(@PathVariable("nome") String nome) {
        return ResponseEntity.ok().body(this.alunoService.findAlunoByNome(nome));
    }

    @GetMapping("/curso/nome/{nomeCurso}")
    public ResponseEntity<?> findAllAlunoByCurso(@PathVariable("nomeCurso") String nomeCurso) {
        return ResponseEntity.ok().body(this.alunoService.findAllAlunoByCurso(nomeCurso));
    }

    @GetMapping("/curso/sigla/{siglaCurso}")
    public ResponseEntity<?> findAllAlunoBySiglaCurso(@PathVariable("siglaCurso") String siglaCurso) {
    ...
    public ResponseEntity<?> findAllAlunoBySemestreTurma(@PathVariable("semestreTurma") Integer semestreTurma) {
        return ResponseEntity.ok().body(this.alunoService.findAllAlunoBySemestreTurma(semestreTurma));
    }

    @GetMapping("/turma/ano/{anoTurma}")
    public ResponseEntity<?> findAllAlunoByAnoTurma(@PathVariable("anoTurma") Integer anoTurma) {
    ...
    public ResponseEntity<?> findAllAlunoByCursoAndTurma(@PathVariable("nomeCurso") String nomeCurso, @PathVariable("anoTurma") Integer anoTurma) {
